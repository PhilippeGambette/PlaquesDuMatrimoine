...
// Write mode
 if($_GET["action"] == "write"){
   ...
   $requestCheckAlias -> execute(array(":nom_complet" => $_GET['nom_complet'], ":alias" => $_GET['alias']));
  ...
  //  Check if 1 or more name appear in the "alias" table
   if(intval($data["nombre_personne"])>= 1){
    ...
    // If false, the data are stocked in database
     $sqlInsertWrite = "INSERT INTO `personne` (`id`, `id_wikidata`, `alias`, `nom_complet`, `genderLabel`, `personDescription`, `sitelink`, `lemma`) VALUES (NULL, :id_wikidata, :alias, :nom_complet, :genderLabel, :personDescription, :siteLink, :lemma)";
     $requestInsertAlias = $db -> prepare($sqlInsertWrite);
     $requestInsertAlias -> execute(array(":id_wikidata" => $_GET['id_wikidata'], ":alias" => $_GET['alias'], ":nom_complet" => $_GET['nom_complet'], ":genderLabel" => $_GET['genderLabel'], ":personDescription" => $_GET['personDescription'], ":siteLink" => $_GET['siteLink'], ":lemma" => $_GET['lemma']));
     $db->exec();
...
// Read mode
  ...
  //  Check if the name is in the database
   $sqlCheckName = "SELECT * FROM alias WHERE nom_potentiel= :nom_potentiel AND prenom_potentiel= :prenomPotentiel";
   $requestCheckName = $db->prepare($sqlCheckName);
   $requestCheckName = execute(array(":nom_potentiel" => $_GET['nom_potentiel'], ":prenom_potentiel" => $_GET['prenom_potentiel']));
